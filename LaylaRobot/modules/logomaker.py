...
async def lego(event):
 ...
 try:
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @katsurobot Support @KatsuSupport")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @KatsuSupport, {e}')
...
async def lego(event):
 ...
 try:
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @katsurobot")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
